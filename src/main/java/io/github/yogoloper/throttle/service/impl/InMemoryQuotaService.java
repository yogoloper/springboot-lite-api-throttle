...
import io.github.yogoloper.throttle.service.QuotaService;
...
public class InMemoryQuotaService implements QuotaService {
    ...
    @Override
    public void useQuota(String key, Quota quota) {
        String quotaKey = buildQuotaKey(key, quota);
        resetIfNeeded(quotaKey, quota);
        quotaUsages.computeIfAbsent(quotaKey, k -> new AtomicInteger(0)).incrementAndGet();
    }
